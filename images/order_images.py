...
def connect_images_db():
    conn=psycopg2.connect("dbname='SafeDrop_OrderImages' user='postgres' password='postgre123' host='localhost' port = '5432'")
    cur=conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS images (url_id SERIAL, TXID TEXT, \
...
def search_allImages(url_id=(None), TXID=(None), UN=(None)):
    conn=psycopg2.connect("dbname='SafeDrop_OrderImages' user='postgres' password='postgre123' host='localhost' port = '5432'")
    cur=conn.cursor()
    cur.execute("SELECT * FROM images WHERE url_id=%s OR TXID=%s OR\
                username=%s",\
                (url_id, TXID, UN))
    rows = cur.fetchall()
    conn.close()
