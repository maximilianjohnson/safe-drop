...
search_ChatValue("access_code","example") #Find the value in the database table
attemptCode(codepull, "example") #check the entered code against the database code
doorVer = codeResult("example") #Check if the code was correct
if (doorVer == )
writeToScaleDoor("example", scale_status = scale)
