...
def create_up_db():
    ...
    cur=conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS users (first_name TEXT,\
    last_name TEXT, user_name TEXT, email TEXT, age INTEGER, date_joined DATE,\
    address TEXT, city TEXT, province TEXT, country TEXT, postal_code TEXT, \
    distance  FLOAT, txA INTEGER, user_rating INTEGER, status TEXT)")
    conn.commit()
    conn.close()
