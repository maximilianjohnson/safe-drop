...
def create_OrderInfo_db():
    ...
    cur=conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS orderInfo (Order_num INTEGER, ID TEXT, B_username TEXT, S_username TEXT, I_name TEXT, date_initialized TEXT, Cost REAL, Location TEXT, status TEXT, date_resolved TEXT)")
    conn.commit()
    conn.close()
...
def newOrder (S_username, B_username, I_name, Cost, Location):
    ...
    #    ID = uuid.uuid4()
    cur.execute("INSERT INTO orderInfo VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (Order_num, ID, B_username, S_username, I_name, date_init, Cost, Location, "incomplete", date_resolved))
    conn.commit()
    conn.close()
